export const SITE_ORIGIN = "https://filoteso.co.in";
...
export const routeSeo = {
  ...
  "/register": {
    ...
    keywords: "filo teso register, create account, streetwear shopping account",
  },
  "/profile": {
...
export const getProductListSeo = (pathname) => {
  const [, , category = "all", subcategory = "all"] =
    normalizeCanonicalPath(pathname).split("/");
  const categoryLabel = categoryLabels[category] || titleCase(category);
  const subcategoryLabel =
    subcategoryLabels[subcategory] || titleCase(subcategory);
...
export const getGenericRouteSeo = (pathname) => {
  const label = titleCase(
    normalizeCanonicalPath(pathname).split("/").filter(Boolean).join(" ")
  );
...
export const getClientRouteSeo = (pathname) => {
  const normalizedPath = normalizeCanonicalPath(pathname);

  if (routeSeo[normalizedPath]) return routeSeo[normalizedPath];
  if (
    normalizedPath === "/products" ||
    normalizedPath.startsWith("/products/")
  ) {
    return getProductListSeo(normalizedPath);
  }

  if (normalizedPath.startsWith("/product/")) {
    const productName = titleCase(normalizedPath.split("/").filter(Boolean)[1]);
    return {
      title: `${productName || "Filo Teso Product"} | Filo Teso`,
      description: `Shop ${productName || "this product"} from Filo Teso, a premium streetwear clothing brand in India.`,
      keywords: `${productName}, filo teso clothing, premium streetwear brand india, streetwear clothing brand india`,
    };
  }

  if (normalizedPath.startsWith("/blog/")) {
    const blogTitle = titleCase(normalizedPath.split("/").filter(Boolean)[1]);
    return {
      title: `${blogTitle || "Filo Teso"} | Filo Teso Blog`,
      description: `Read ${blogTitle || "this streetwear guide"} on the Filo Teso blog for modern streetwear fashion, styling ideas, and clothing inspiration.`,
      keywords: `${blogTitle}, filo teso blog, streetwear fashion, modern streetwear fashion, premium streetwear brand india`,
    };
  }

  return getGenericRouteSeo(normalizedPath);
};
